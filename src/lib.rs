...
pub fn image(img: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    handle_error(|| {
        ...
        // The path for the documentation generated by Cargo has changed in the newer versions.
        let output = format!("</p><img src=\"../../images/{}\"/>", img);
...
fn save_plantuml(uml_str: &str) -> String {
    ...
    let uml_file_name =
        &("./target/doc/images/puml_files/".to_string() + hasher.result_str().as_str() + ".png");

    let svg = download_puml_svg(uml_str);

    if !svg.is_empty() {
        output.push_str(&("<p><div>".to_string() + svg.as_str() + "</div></p>"));
    } else if save_http_png::download_puml(uml_str, uml_file_name) {
        //The path for the documentation generated by Cargo has changed in the newer versions.
        ...
        println!("warning: failed to download the plantuml picture, use dynamic hyperlink instead");
